...
export function TaskLabel({task}:{task:Task}) {
  ...
  return (
    <article id={task.id} data-category={task.l_category as EMCategory} className="relative rounded bg-white font-bold my-2 p-2 text-sm text-habitxt w-full order-1" ref={setNodeRef} style={style} {...listeners} {...attributes}>
      <h3>{ task.name }</h3>
